...
const Computers = ({ isMobile }) => {
  ...
  useFrame((state, delta) => {
    ...
    if (!isUserInteracting && modelRef.current) {
      ...
      modelRef.current.position.y = Math.sin(autoRotateRef.current.time * 0.5) * 0.05 - 4; // Lowered position from -2.25 to -2.75
    }
  });
  ...
  return (
    <mesh>
      ...
      <group 
        ...
        position={isMobile ? [0, -2.75, -2.2] : [0, -2.75, -1.5]} // Lowered position from -2.25 to -2.75
        scale={isMobile ? 0.85 : 0.9} // Current scale
